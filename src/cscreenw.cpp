...
CScreenW::CScreenW( QWidget *parent ) :
    QDialog( parent ),
    ui( new Ui::CScreenW )
{
    ui->setupUi(this);
...
*/
    ...
    for( int j = 0; j < 64; j++ ) for( int k = 0; k < 64; k++ ) for( x = 0; x < 256*3; x++ ) m_Screen[j][k][x] = 0;
...
//	m_DragDrop = false;
    ...
    ui->m_pGrNothing->setVisible( false );

    connect( ui->m_pGrTilesBlockW, SIGNAL( valueChanged( int)), this, SLOT( OnQuitFocus( int )));
    connect( ui->m_pGrTilesBlockH, SIGNAL( valueChanged( int)), this, SLOT( OnQuitFocus( int )));
...
void CScreenW::OnBank0ClickRight( int x, int y ) {
    OnBankClickRight( x, y, 0 );

}

void CScreenW::OnBank1ClickRight( int x, int y ) {
    OnBankClickRight( x, y, 1 );
}

void CScreenW::OnBank2ClickRight( int x, int y ) {
    OnBankClickRight( x, y, 2 );
}
...
bool CScreenW::ExportScreenBinPletter( QString fileName, int initX, int initY, int w, int h )
{
    ...
    for( mapY = 0; mapY < ui->m_pGrMapHeight->value(); mapY++ )
    {
        ...
        //  PletterMain( s, m_Buffer, qPrintable( QString( "%1-%2-%3").arg( fileName ).arg( mapX ).arg( mapY )) );
            PletterMain( s, m_Buffer, QString( "%1-%2-%3").arg( fileName ).arg( mapX ).arg( mapY ) );
        }
    }
...
bool CScreenW::ExportScreenData( QString fileName, bool hexa, int initX, int initY, int w, int h )
{
    ...
    for( mapY = 0; mapY < ui->m_pGrMapHeight->value(); mapY++ )
    {
        for( mapX = 0; mapX < ui->m_pGrMapWidth->value(); mapX++ )
        {
            str<<endl<<QString( "SCREEN-%1-%2:" ).arg( mapX ).arg( mapY )<<endl;
            str<<"db ";
            for( y = initY; y < he; )
            {
                for( x = initX; x < wi; )
...
void CScreenW::ExchangeTiles( int tiles1, int tiles2 )
{
    ...
    for( int b = 0; b < 3; b++ )
    {
        if( !ui->m_pGrOneBank->isChecked() && m_pTabBank->currentIndex() != b )
            continue;
...
bool CScreenW::ExportPaletteASM( QString fileName ) {
    ...
    str << "PALETTE:" << endl;
    str << "; --------------------------------------------------------\n";
    str << ";    [0~15] Decimal [0~7]      Binary VDP      Hexadecimal\n";
    str << ";   DB    NN, R, G, B    ; 0RRR0BBB 00000GGG ; 0xAARRGGBB \n";
    str << "; --------------------------------------------------------\n";
    for( int i = 0; i < 16; ++i ) {
        int j = ( i < 15 ) ? i + 1 : 0 ;
        ...
        // Hexadecimal:
        s += QString( "0x%1\n" ).arg( color.rgb(), 8, 16, QChar( '0' ) ).toUpper();
        // print
        str << s;
    }
    hFile.flush();
...
bool CScreenW::ExportPaletteVDP( QString fileName ) {
    ...
    uint r, g, b;
    str << "; ----------------------------------------------\n";
    str << ";    [0~15]         Binary           Hexadecimal\n";
    str << "; ColorNN:    0RRR0BBB 00000GGG    ; 0xAARRGGBB \n";
    str << "; ----------------------------------------------\n";
    for( int i = 0; i < 16; ++i ) {
